...
def verificar_credenciales(usuario: str, password: str) -> bool:
    """Verifica usuario y contraseña contra Streamlit Secrets."""
    try:
        usuarios_validos = st.secrets["AUTH_USERS"].split(",")
        password_hash = st.secrets["AUTH_PASSWORD_HASH"]
        return usuario.strip() in [u.strip() for u in usuarios_validos] \
               and hash_password(password) == password_hash
    except Exception:
        return False
...
# ============================================================
#  CONTROL DE SESIÓN — bloquea todo si no está autenticado
# ============================================================
...
if not st.session_state["autenticado"]:
    mostrar_login()
    st.stop()  # Nada más se ejecuta hasta que haya login exitoso
...
# ============================================================
#  INTERFAZ PRINCIPAL (solo visible tras login exitoso)
# ============================================================
...
with col_usuario:
    ...
    st.markdown(f"<div class='user-badge'>👤 {st.session_state['usuario_actual']}</div>",
                unsafe_allow_html=True)
...
if df is None:
    st.warning("⚠️ No se pudieron cargar los datos. Verificá la configuración.")
    st.stop()
...
campo = st.selectbox(
    ...
    format_func=lambda x: {
        "RUB": "🏠 RUB — Código de rubro",
        "CI TIT": "👤 CI TIT — Cédula del titular",
        "CI SEC": "👥 CI SEC — Cédula del secundario"
    }[x]
)

valor_input = st.text_input(
    f"Ingresá el valor de {campo}:",
...
if buscar:
    if not valor_input.strip():
        ...
        if len(resultados) > 0:
            ...
            nombre_archivo = f"Resultados_{campo}_{valor}_{fecha_hora}.xlsx"
